...
# If no package manager is needed, don't include package manager
# specific directory structures and registers.
BOOTSTRAP_NO_PACKAGE_MANAGER=false
	...
	if [ ${TERMUX_PACKAGE_MANAGER} = "apt" ]; then
		...
		if [ ! -e "$package_tmpdir/package.deb" ]; then
			...
			(cd "$package_tmpdir"
				...
				if ! ${BOOTSTRAP_ANDROID10_COMPATIBLE} || ! ${BOOTSTRAP_NO_PACKAGE_MANAGER}; then
